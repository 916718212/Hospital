...
<head>
		...
		<link rel="stylesheet" type="text/css" href="<%=basePath%>./WebContent/css/emergencyRescue.css"/>
...
<body>
	<!-- 导航栏 -->
		<div class="layui-row">
			<div class="layui-col-lg10">
					<ul class="layui-nav layui-bg-green" lay-filter="">
					  ...
					  <li class="layui-nav-item layui-this"><a href="emergencyRescue.jsp?emergencyrescue=">紧急救援</a></li>
		...
		<div class="button_line">
			<form action="emergencyRescue.jsp" method="post">
				<input type="text" name="emergencyrescue" /> 
		...
		<div class="bg-white">
			<div class="developer-toolbag-block">
				<div class="AGrid-row  tool-package">
					...
					<!--使用EmergencyRescueDao-->
					<jsp:useBean id="eRDao" class="dao.EmergencyRescueDao"></jsp:useBean>
					
					
					<%
					String emergencyrescue = (String)request.getParameter("emergencyrescue");
					ResultSet rs = null;
					if(emergencyrescue.equals("")){rs = eRDao.getEmergencyRescue("SELECT * FROM emergencyrescue");}
					else{rs = eRDao.getEmergencyRescue("SELECT * FROM emergencyrescue WHERE name LIKE '%"+emergencyrescue+"%'");}
					while(rs.next()){
					 %>
					 <!--循环体-->
					<div class="col-sm-6 ">
						<div class="tile center-layer" style="height:230px;">
							<div class="layer-figure">
								<img src="<%=basePath%>./WebContent/images/logo.png">
							</div>
							<h2 class="dev-list"><%=rs.getString("name") %></h2>
		
							<div class="layer-text">
								<p class="dev-text"><%=rs.getString("detail") %></p>
							</div>
						</div>
					</div>
					<%} %>
