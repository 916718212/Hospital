...
<body>
	...
	<!-- 判断删除状态 -->
	<%
	String status_delete_messageboard = (String)session.getAttribute("status_delete_messageboard");
	if(status_delete_messageboard==null){
		status_delete_messageboard = "0";
	}
	//删除成功
	if(status_delete_messageboard.equals("1")){out.println("<script type=\"text/javascript\">"); 
								  out.println("alert(\"删除成功！\");"); 
								  out.println("</script>");}
	session.setAttribute("status_delete_messageboard","0"); 
	%>
	<!-- 判断删除状态 -->
                    ...
                    <div class="am-g">
                        ...
                        <div class="am-u-sm-12 am-u-md-3">
                            <div class="am-input-group am-input-group-sm">
                                <form action="manager_MessageBoard.jsp" method="post">
                                    ...
                                    <tbody>
										<jsp:useBean id="mBDao" class="dao.MessageBoardDao"></jsp:useBean>
										  <%ResultSet rs = null;
											...
											if("".equals(search)){rs = mBDao.getMessageBoard("SELECT * FROM messageboard");}
											else{rs = mBDao.getMessageBoard("SELECT * FROM messageboard WHERE name LIKE '%"+search+"%'");}
											while(rs.next()){ %>
                                        <tr>
                                        <%if(rs.getString("message").length()>15){ %>
                                            <td><%=rs.getString("message").substring(0,15) %></td>
                                            <%}else{ %>
                                            ...
                                            <td>
                                                <div class="am-btn-toolbar">
                                                    <div class="am-btn-group am-btn-group-xs">
                                                        <a href="servlet/DeleteMessageBoard?delete_time=<%=rs.getString("time") %>" class="am-btn am-btn-default am-btn-xs am-text-danger am-hide-sm-only"><span class="am-icon-trash-o"></span> 删除</a>
                                                    </div>
                                                </div>
                                            </td>
                                        </tr>
