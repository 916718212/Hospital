...
<body>
	...
	<!-- 判断新增状态 -->
	<%
	String status_add_customer = (String)session.getAttribute("status_add_customer");
	if(status_add_customer==null){
		status_add_customer = "0";
	}
	//新增成功
	if(status_add_customer.equals("1")){out.println("<script type=\"text/javascript\">"); 
								  out.println("alert(\"添加成功！\");"); 
								  out.println("</script>");}
	session.setAttribute("status_add_customer","0"); 
	%>
	<!-- 判断新增状态 -->
	
	<!-- 判断删除状态 -->
	<%
	String status_delete_customer = (String)session.getAttribute("status_delete_customer");
	if(status_delete_customer==null){
		status_delete_customer = "0";
	}
	//删除成功
	if(status_delete_customer.equals("1")){out.println("<script type=\"text/javascript\">"); 
								  out.println("alert(\"删除成功！\");"); 
								  out.println("</script>");}
	session.setAttribute("status_delete_customer","0"); 
	%>
	<!-- 判断删除状态 -->
	
	<!-- 判断更新状态 -->
	<%
	String status_update_customer = (String)session.getAttribute("status_update_customer");
	if(status_update_customer==null){
		status_update_customer = "0";
	}
	//更新成功
	if(status_update_customer.equals("1")){out.println("<script type=\"text/javascript\">"); 
								  out.println("alert(\"更新成功！\");"); 
								  out.println("</script>");}
	session.setAttribute("status_update_customer","0"); 
	%>
	<!-- 判断更新状态 -->
            ...
            <div class="tpl-portlet-components">
                <div class="portlet-title">
                    <div class="caption font-green bold">
                        <span class="am-icon-code"></span> 顾客信息
                    </div>
                    ...
                    <div class="am-g">
								...
								<div class="morph-content">
									<div>
										<div class="content-style-form content-style-form-1">
											...
											<h2>新增</h2>
											<form style="margin-top:-50px;" action="servlet/AddCustomer" method="post">
											<p><label>用户名</label><input type="text" name="add_name"/></p>
											<p><label>账号</label><input type="text" name="add_account" /></p>
											<p><label>密码</label><input type="text" name="add_password" /></p>
                        ...
                        <div class="am-u-sm-12 am-u-md-3">
                            <div class="am-input-group am-input-group-sm">
                                <form action="manager_Customer.jsp" method="post">
                            ...
                            <form class="am-form">
                                ...
                                <table class="am-table am-table-striped am-table-hover table-main">
                                    ...
                                    <tbody>
										...
										if("".equals(search)){rs = uDao.getCustomer("SELECT * FROM customer");}
										else{rs = uDao.getCustomer("SELECT * FROM customer WHERE account LIKE '%"+search+"%'");}
										while(rs.next()){ %>
                                        <tr>
                                            <td><%=rs.getString("name") %></td>
                                            <td class="am-hide-sm-only"><%=rs.getString("account") %></td>
                                            <td class="am-hide-sm-only"><%=rs.getString("password") %></td>
                                            <td>
                                                <div class="am-btn-toolbar">
                                                    <div class="am-btn-group am-btn-group-xs">
															...
															<div class="morph-content">
																<div>
																	<div class="content-style-form content-style-form-1">
																		...
																		<form action="servlet/UpdateCustomer" method="post" >
																		<p><label>用户名</label><input type="text" value=<%=rs.getString("name") %> name="update_name" /></p>
																		<p><label>账号</label><input type="text" value=<%=rs.getString("account") %> name="update_account"/></p>
																		<p><label>密码</label><input type="text" value=<%=rs.getString("password") %> name="update_password" /></p>
																		<p><input type="submit" value="保存" style="background-color:E75854;color:white;width:100%;height:55px;margin-top:20px;" /></p>
																		</form>
																		</div>
																	</div>
                                                        ...
                                                        <a href="servlet/DeleteCustomer?delete_account=<%=rs.getString("account") %>" class="am-btn am-btn-default am-btn-xs am-text-danger am-hide-sm-only"><span class="am-icon-trash-o"></span> 删除</a>
                                                    </div>
                                                </div>
                                            </td>
                                        </tr>
