...
<body>
	...
	<!-- 判断新增状态 -->
	<%
	String status_add_work = (String)session.getAttribute("status_add_work");
	if(status_add_work==null){
		status_add_work = "0";
	}
	//新增成功
	if(status_add_work.equals("1")){out.println("<script type=\"text/javascript\">"); 
								  out.println("alert(\"添加成功！\");"); 
								  out.println("</script>");}
	session.setAttribute("status_add_work","0"); 
	%>
	<!-- 判断新增状态 -->
	
	<!-- 判断删除状态 -->
	<%
	String status_delete_work = (String)session.getAttribute("status_delete_work");
	if(status_delete_work==null){
		status_delete_work = "0";
	}
	//删除成功
	if(status_delete_work.equals("1")){out.println("<script type=\"text/javascript\">"); 
								  out.println("alert(\"删除成功！\");"); 
								  out.println("</script>");}
	session.setAttribute("status_delete_work","0"); 
	%>
	<!-- 判断删除状态 -->
	
	<!-- 判断更新状态 -->
	<%
	String status_update_work = (String)session.getAttribute("status_update_work");
	if(status_update_work==null){
		status_update_work = "0";
	}
	//更新成功
	if(status_update_work.equals("1")){out.println("<script type=\"text/javascript\">"); 
								  out.println("alert(\"更新成功！\");"); 
								  out.println("</script>");}
	session.setAttribute("status_update_work","0"); 
	%>
	<!-- 判断更新状态 -->
                    ...
                    <div class="am-g">
								...
								<div class="morph-content">
									<div>
										<div class="content-style-form content-style-form-1">
											...
											<h2>新增</h2>
											<form style="margin-top:-50px;" action="servlet/AddWork" action="post">
											<p><label>任务内容</label><input type="text" name="add_detail"/></p>
											<p><label>结束时间(yyyy-MM-dd HH:mm:ss)</label><input type="text" name="add_end"/></p>
                        ...
                        <div class="am-u-sm-12 am-u-md-3">
                            <div class="am-input-group am-input-group-sm">
                                <form action="manager_Work.jsp" method="post">
                        ...
                        <div class="am-u-sm-12">
                            <form class="am-form">
                                ...
                                <table class="am-table am-table-striped am-table-hover table-main">
                                    <thead>
                                        <tr>
                                            <th class="table-id">内容</th>
                                            <th class="table-title">状态</th>
                                            <th class="table-type">起始日期</th>
                                            <th class="table-type">截止日期</th>
                                            <th class="table-date am-hide-sm-only">完成日期日期</th>
                                    ...
                                    <tbody>
										<jsp:useBean id="WDao" class="dao.WorkDao"></jsp:useBean>
										  <%ResultSet rs = null;
											...
											if("".equals(search)){rs = WDao.getWork("SELECT * FROM work");}
											else{rs = WDao.getWork("SELECT * FROM work WHERE detail LIKE '%"+search+"%'");}
											while(rs.next()){ %>
                                        <tr>
                                            ...
                                            <td class="am-hide-sm-only"><%=rs.getString("finish") %></td>
                                            <td>
                                                <div class="am-btn-toolbar">
                                                    <div class="am-btn-group am-btn-group-xs">
															...
															<div class="morph-content">
																<div>
																	<div class="content-style-form content-style-form-1">
																		...
																		<form action="servlet/UpdateWork" method="post" >
																		<p><label>任务内容</label><input type="text" name="update_detail" value=<%=rs.getString("detail") %>></p>
																		<p><label>状态('进行中','未完成','已完成')</label><input type="text" name="update_status" value=<%=rs.getString("status") %> ></p>
																		<p><label>结束时间(yyyy-MM-dd HH:mm:ss)</label><input type="text" name="update_end" value="<%=rs.getString("end")%>" ></p>
																		<p><input type="submit" value="保存" style="background-color:E75854;color:white;width:100%;height:55px;margin-top:20px;" /></p>
																		</form>
																		</div>
																	</div>
                                                        ...
                                                        <a href="servlet/DeleteWork?delete_detail=<%=rs.getString("detail") %>" class="am-btn am-btn-default am-btn-xs am-text-danger am-hide-sm-only"><span class="am-icon-trash-o"></span> 删除</a>
                                                    </div>
                                                </div>
                                            </td>
                                        </tr>
