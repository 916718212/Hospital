...
public class DeleteExpertCommunity extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			md.addMessage("删除专家成功", account);
			//将删除状态存入session中,若为-1，则为失败；为0，无删除状态；为1，则为成功；
			request.getSession().setAttribute("status_delete_expertcommunity","1");
			eCDao.deleteExpertCommunity(name);
