...
public class DeleteCustomer extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		String account = new String(request.getParameter("delete_account").getBytes("iso-8859-1"), "utf-8");
		UserDao uDao = new UserDao();
		try {
			MessageDao md = new MessageDao();
			String account0 = (String)request.getSession().getAttribute("account");
			md.addMessage("删除顾客成功", account0);
			//将删除状态存入session中,若为-1，则为失败；为0，无删除状态；为1，则为成功；
			request.getSession().setAttribute("status_delete_customer","1");
			uDao.deleteCustomer(account);
