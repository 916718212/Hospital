...
public class AddExpertCommunity extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			md.addMessage("添加专家成功", account);
			//将添加状态存入session中,若为-1，则为失败；为0，无添加状态；为1，则为成功；
			request.getSession().setAttribute("status_add_expertcommunity","1");
			eCDao.addExpertCommunity(expertcommunity);
